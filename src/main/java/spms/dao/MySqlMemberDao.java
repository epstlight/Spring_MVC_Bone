...
public class MySqlMemberDao implements MemberDao {
  ...
  public List<Member> selectList(HashMap<String,Object> paramMap) 
    ...
    try {
      return sqlSession.selectList("Member.selectList", paramMap);
    } finally {
      sqlSession.close();
    }
  }
  ...
  public int update(Member member) throws Exception { 
    ...
    try {
    	...
    	if (paramMap.size() > 0) {
    		...
    		int count = sqlSession.update("Member.update", paramMap);
    		sqlSession.commit();
    		return count;
    	} else {
    		return 0;
  ...
  public int delete(int no) throws Exception {  
  	SqlSession sqlSession = sqlSessionFactory.openSession();
    try {
      int count = sqlSession.delete("Member.delete", no);
      sqlSession.commit();
      return count;
    } finally {
      sqlSession.close();
  ...
  public Member exist(String email, String password) throws Exception {
  	...
  	SqlSession sqlSession = sqlSessionFactory.openSession();
  	try {
  		return sqlSession.selectOne("Member.exist", paramMap);
  	} finally {
  		sqlSession.close();
  	}
  }
